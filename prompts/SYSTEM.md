...
Плохо: (пустой content), «Выполняю инструмент», «Читаю файл»
Хорошо: «Ищу функцию handle_task в agent.py — нужно понять как
собирается контекст перед LLM-вызовом. Если найду, смогу оптимизировать.»
Хорошо: «Push не прошёл — конфликт с удалённой веткой. Сделаю pull --rebase
и попробую снова. Если не поможет — проверю git status.»
